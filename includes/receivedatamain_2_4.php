...
$sql = "INSERT INTO timestamp_2_4 (user_id) VALUES ('$user_id');";
//echo $sql;
...
$sql = "SELECT MIN(click_id) AS min FROM timestamp_2_4 WHERE user_id = $user_id;";
...
for ($i=$lowestclickID; $i<=($lowestclickID + count($arrayData)); $i++ ) {
  $sql = "UPDATE timestamp_2_4 SET click_timestamp = $arrayData[$m] WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  $m++;

  $sql = "UPDATE timestamp_2_4 SET click_number = $n WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  //echo "DONE ";
  $n++;
}
...
if($user_id % 2 == 0){ 
		...
		else if ($_COOKIE["timeofappereance1"] == 0 && $_COOKIE["timeofappereance2"] == 1 && $_COOKIE["timeofappereance3"] == 0) {
