...
$arrayData = $_REQUEST['arrayData12'];
...
$sql = "INSERT INTO timestamp_3_4 (user_id) VALUES ('$user_id')";
//echo $sql;
...
$sql = "SELECT MIN(click_id) AS min FROM timestamp_3_4 WHERE user_id = $user_id;";
...
for ($i=$lowestclickID; $i<=($lowestclickID + count($arrayData)); $i++ ) {
  $sql = "UPDATE timestamp_3_4 SET click_timestamp = $arrayData[$m] WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  $m++;

  $sql = "UPDATE timestamp_3_4 SET click_number = $n WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  //echo "DONE ";
  $n++;
}
...
if($user_id % 2 == 0){ 
		...
		if ($_COOKIE["timeofappereance1"] == 1 && $_COOKIE["timeofappereance2"] == 0 && $_COOKIE["timeofappereance3"] == 1) {
			$cookie2_value = "1";	
			$cookie3_value = "1";
			$cookie4_value = "1";
		...
		else if ($_COOKIE["timeofappereance1"] == 0 && $_COOKIE["timeofappereance2"] == 1 && $_COOKIE["timeofappereance3"] == 1)  {
			$cookie2_value = "1";
			$cookie3_value = "1";
			$cookie4_value = "1";
			setcookie($cookie3_name, $cookie3_value, time() + (86400 * 30), "/"); // 86400 = 1 day
		...
		else if ($_COOKIE["timeofappereance1"] == 0 && $_COOKIE["timeofappereance2"] == 0 && $_COOKIE["timeofappereance3"] == 1) {
