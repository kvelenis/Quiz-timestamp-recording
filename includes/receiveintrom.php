...
//echo $user_id;
//echo $arrayData;
$sql = "UPDATE users SET heardbefore_introm = '$heardbefore' WHERE user_id = $user_id;";
mysqli_query($conn, $sql);

for ($i=0; $i<count($arrayData); $i++) {
$sql = "INSERT INTO timestamp_intro_midi (user_id) VALUES ('$user_id')";
//echo $sql;
mysqli_query($conn, $sql);
}

$sql = "SELECT MIN(click_id) AS min FROM timestamp_intro_midi WHERE user_id = $user_id;";
...
for ($i=$lowestclickID; $i <= ($lowestclickID + count($arrayData)); $i++ ) {
  $sql = "UPDATE timestamp_intro_midi SET click_timestamp = $arrayData[$m] WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  $m++;

  $sql = "UPDATE timestamp_intro_midi SET click_number = $n WHERE user_id= $user_id AND click_id = $i;";
  mysqli_query($conn, $sql);
  //echo "DONE ";
  $n++;
}
